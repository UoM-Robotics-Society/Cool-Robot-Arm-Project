...
bool BFGS::Run(double x, double y, double z, LA::vecd<5> current_motors,
               LA::vecd<5>& output_motors) {
    Reset();
    X = current_motors;
    ls.set_goal(x, y, z);
    int count = 0;
    ...
    while (true) {
        ...
        while (cost > COST_THRESHOLD && count < 20) {
            ...
            if (debugIteration) PrintIteration();

            if (mu < MU_BROKEN &&
                ls.dist_to_goal(X, 0) < DIST_THRESHOLD * 2 &&
                ls.InBounds(X)) {
                print_debug(
                ...
                print_debug(
                    "ERROR: Could not reach goal within mu time frame\n");
                output_motors = X;
                return false;
            }
            if (std::isnan(P[0])) {
                print_debug("ERROR: Stuck in local minima/singularity\n");
                output_motors = X;
                return false;
            }

            Xp = ls.GoldenSearch(X, P, mu);
            // costDiff = abs(ls.cost_function(X,0,mu) -
            // ls.cost_function(Xp,0,mu));
            S = Xp - X;
            ...
            // (((S.t()*Y.t() +
            // Y.t()*Bi*Y)*(S*S.t()))/(arma::powmat((S.t()*Y.t()),2)))-(((Bi*Y*S.t())+(S*Y.t()*Bi))/(S.t()*Y));

            LA::matd<1, 5> St = LA::transpose(S);
            LA::matd<1, 5> Yt = LA::transpose(Y);
            LA::matd<5, 5> a =
                (LA::as_scalar(St * Y) + LA::as_scalar((Yt * Bi) * Y)) *
                (S * St);
