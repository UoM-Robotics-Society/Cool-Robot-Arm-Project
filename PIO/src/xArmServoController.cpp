...
xArmServoController::xArmServoController(xArmMode mode, Stream &serial_port)
...
void xArmServoController::setPosition(int servo_id, unsigned position,
...
void xArmServoController::setPosition(xArmServo servo,
                                      unsigned duration /*= 1000*/,
                                      bool wait /* = false*/) {
    setPosition(servo.servo_id, servo.position, duration, wait);
}

void xArmServoController::setPosition(xArmServo servos[], int count,
                                      unsigned duration /*= 1000*/,
                                      bool wait /* = false*/) {
    int len = count * 3 + 3;
    _buffer[0] = count;
...
void xArmServoController::actionStop() { send(CMD_ACTION_GROUP_STOP, 0); }
...
bool xArmServoController::actionIsRunning() { return actionRunning; }
...
void xArmServoController::beep() { serial_port.write((byte)CMD_BEEP); }